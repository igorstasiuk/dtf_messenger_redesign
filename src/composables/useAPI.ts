...
export function useAPI() {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (response.success && response.result) {
          ...
          if (
            response.error &&
            (response.error.code === 401 || response.error.code === 403)
          ) {
          ...
          if (
            attempt < retries &&
            response.error &&
            (response.error.code === 0 || response.error.code >= 500)
          ) {
  ...
  const channelsAPI = {
    getChannels: () =>
      makeAPICall("channels-list", () => dtfAPI.getChannels(), {
        showErrorNotification: true,
      }),

    getChannel: (id: number) =>
      makeAPICall(`channel-${id}`, () => dtfAPI.getChannel(id), {
        showErrorNotification: true,
      }),

    createChannelWithUser: (userId: number) =>
      makeAPICall(
  ...
  const messagesAPI = {
    getMessages: (channelId: number, beforeTime?: number, limit?: number) =>
      makeAPICall(
        `messages-${channelId}`,
    ...
    sendMessage: (channelId: number, text: string, _media: File[] = []) =>
      makeAPICall(
    ...
    markAsRead: (channelId: number, messageIds: number[]) =>
      makeAPICall(
        `mark-read-${channelId}`,
  ...
  const mediaAPI = {
    uploadFile: (file: File) =>
      makeAPICall(`upload-${file.name}`, () => dtfAPI.uploadFile(file), {
        showErrorNotification: true,
  ...
  const userAPI = {
    ...
    getUserProfile: () =>
      makeAPICall("user-profile", () => dtfAPI.getUserProfile(), {
        showErrorNotification: true,
      }),
  };
  ...
  async function healthCheck() {
    return makeAPICall("health-check", () => dtfAPI.healthCheck(), {
      showErrorNotification: false,
      retries: 0,
    });
  }
