<template>
  <div class="chat-sidebar">
    ...
    <div class="chat-sidebar__header">
      <div v-if="activeChannel" class="chat-sidebar__channel-info">
        <div class="chat-sidebar__channel-avatar">
          <img
            v-if="activeChannel.picture"
            :src="activeChannel.picture"
            :alt="activeChannel.title"
            class="w-8 h-8 rounded-full object-cover"
          >
          <div v-else class="w-8 h-8 rounded-full bg-dtf-orange text-white flex items-center justify-center text-sm font-medium">
            {{ getChannelInitials(activeChannel.title) }}
          </div>
        </div>
        <div class="chat-sidebar__channel-details">
          <h2 class="chat-sidebar__channel-name">{{ activeChannel.title }}</h2>
          <p class="chat-sidebar__channel-meta">
            {{ (activeChannel as any).participantCount || 0 }} участников
          </p>
        </div>
      </div>
    ...
    <div class="chat-sidebar__content">
      <div v-if="!activeChannel" class="chat-sidebar__empty">
        <div class="text-center py-12">
          ...
          <h3 class="text-lg font-medium text-gray-900 mb-2">Выберите канал</h3>
          <p class="text-gray-500">Выберите канал из списка, чтобы начать общение</p>
        </div>
      </div>

      <div v-else class="chat-sidebar__messages-container">
        <!-- Loading -->
        <div v-if="messagesStore.isLoading" class="chat-sidebar__loading">
          <LoadingSpinner />
          ...
          >
            <div class="chat-sidebar__message-avatar">
              <img
                ...
                class="w-8 h-8 rounded-full object-cover"
              >
              <div v-else class="w-8 h-8 rounded-full bg-gray-400 text-white flex items-center justify-center text-sm">
            ...
            <div class="chat-sidebar__message-content">
              <div class="chat-sidebar__message-header">
                <span class="chat-sidebar__message-author">{{ message.author.name }}</span>
                <span class="chat-sidebar__message-time">{{ formatMessageTime(message.dtCreated) }}</span>
              </div>
              
              <div class="chat-sidebar__message-body">
                <p v-if="message.type === 'text' || !message.type" class="chat-sidebar__message-text">
                  {{ message.text }}
                </p>
                <div v-else-if="message.type === 'media' && message.media && message.media[0]?.type === 'image'" class="chat-sidebar__message-image">
                  <img :src="message.media[0]?.data?.url" :alt="message.text" class="max-w-xs rounded-lg" />
                  <p v-if="message.text" class="mt-1">{{ message.text }}</p>
                </div>
                <div v-else-if="message.type === 'media' && message.media" class="chat-sidebar__message-file">
                  <div class="flex items-center p-3 bg-gray-100 rounded-lg">
                    ...
                    <span>{{ message.text || message.media[0]?.data?.name || 'Файл' }}</span>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div v-if="activeChannel" class="chat-sidebar__input">
      <div class="chat-sidebar__input-container">
        ...
        <button
          ...
          @click="sendMessage"
        >
          <LoadingSpinner v-if="isSending" small />
...
function getTypingText(users: readonly { readonly id: number; readonly name: string; readonly avatar: string; readonly startedAt: number; }[]): string {
  if (users.length === 1) {
